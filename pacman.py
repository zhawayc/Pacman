...
class Pacman(GameCharacter):
    ...
    def update(self):
        ...
        if(self.open_or_close == 1
           and self.mouth_angle <= self.max_mouth_angle):
            self.open_close = self.mouth_speed
        elif (self.open_or_close == 1
              and self.mouth_angle > self.max_mouth_angle):
            self.open_or_close = 0
            self.open_close = -self.mouth_speed
        elif (self.open_or_close == 0
              and self.mouth_angle < self.min_mouth_angle):
            self.open_or_close = 1
            self.open_close = self.mouth_speed
        else:
            self.open_close = -self.mouth_speed
        ...
        if(self.x < self.CHAR_WIDTH/2):
            self.maze.eat_dots(self.maze.WIDTH + self.x, self.y)
        elif(self.x + self.CHAR_WIDTH/2 > self.maze.WIDTH):
            self.maze.eat_dots(self.x-self.maze.WIDTH, self.y)
        elif(self.y < self.CHAR_WIDTH/2):
            self.maze.eat_dots(self.x, self.maze.HEIGHT+self.y)
        elif(self.y+self.CHAR_WIDTH/2 > self.maze.HEIGHT):
